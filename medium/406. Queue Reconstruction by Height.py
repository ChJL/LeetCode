...
SOL:
1. sort by second element(increasing)
2. sort by first elemnet (decreasing)
The insert position would exactly be the 2nd element in the each people[i]
Take a look at : https://youtu.be/JtJVMhs-6aw?t=1484

'''

class Solution:
    def second_ele(self,e):
        return e[1]
    def first_ele(self,e):
        return e[0]
    
    def reconstructQueue(self, people: List[List[int]]) -> List[List[int]]:
        if len(people) < 2:
            return people
        people.sort(key=self.second_ele)
        people.sort(key=self.first_ele, reverse = True)
